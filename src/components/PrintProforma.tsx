...
// ADD props HERE 👇
export default function PrintProforma({ goBack }: PrintProformaProps) {
  ...
  const handleFetch = async () => {
    try {
      ...
      if (!txns.transactions?.length) throw new Error('No asset creation transaction found')
      const creationTxn = txns.transactions[0]
      if (!creationTxn.note) throw new Error('No note found')
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {/* OPTIONAL BACK BUTTON USING PROP */}
      <button
        onClick={goBack}
      ...
      {data && (
        <>
          <div id="proforma-card" className="bg-white shadow-lg p-6">
            ...
            <p><strong>Seat No:</strong> {data.seatNumber}</p>
              ...
              <tfoot>
                {(() => {
                  ...
                  return (
                    <>
                      <tr>
                        <td colSpan={3} className="border px-2 font-bold">Total</td>
                        <td className="border px-2">{total}</td>
                        <td className="border px-2">{avgGPA.toFixed(2)}</td>
                      </tr>

                      <tr>
                        <td colSpan={3} className="border px-2 font-bold">Percentage</td>
                      ...
                      <tr>
                        <td colSpan={3} className="border px-2 font-bold">CGPA</td>
                        <td className="border px-2" colSpan={2}>{cgpa}</td>
                      </tr>

                      <tr>
                        <td colSpan={5} className="px-2 py-3 italic">
                          Mr./Ms. {data.studentName} has secured {numberToWords(rounded)} percent with a GPA of {cgpa} in {data.semester}ᵗʰ semester.
                        </td>
                      </tr>
                    </>
                  )
